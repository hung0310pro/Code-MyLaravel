...
Route::get('query/count', function(){
	$data = DB::table('sanphammigrate')->count();
	/*->select(DB::raw('count(*) as count'))*/
	echo "số dòng là : ".$data;
});

// use DB::raw
Route::get('query/dbrawcount', function(){
	$data = DB::table('sanphammigrate')->select(DB::raw('count(*) as count1'))->get();
	/*->select(DB::raw('count(*) as count'))*/
	foreach ($data as $key => $value) {
	echo "<pre>";
	print_r($value->count1);
	echo "</pre>";
	}
...
// +++ cache : Cache hay còn được gọi là bộ nhớ đệm về cơ bản là một lớp ở giữa website và cơ sở dữ liệu của bạn(cx là 1 nơi lưu trữ dữ liệu như kiểu session ấy, tuy nhiên cái này sẽ thường được dùng lưu dữ liệu tạm thời cho api trả về) https://techblog.vn/cache-trong-laravel-5
...
-- Phương thức havingRaw có thể được sử dụng thiết lập các chuỗi vào mệnh đề having
